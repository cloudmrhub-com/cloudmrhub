...
image_obj = mapVBVD(singleraidnoisefile);
...
[singleR_noisecov,singleR_noise_bandwidth] = calc_noise_cov(singleR_noisedata,1);
clear image_obj kdata

% READ noise prescan from multi RAID file
image_obj = mapVBVD(multiraidfile);
...
knoise = knoise(:,:,:,1,1,1);
knoise = permute(knoise,[1 3 2]);
knoise = knoise*correction_factor;
% knoise2 = prescan_obj.noise();
% knoise2 = knoise2(:,:,:,1,1,2);
% knoise2 = permute(knoise2,[1 3 2]);
% knoise2 = knoise2*correction_factor;
