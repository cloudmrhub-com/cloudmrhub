% -------------------------------------------------------------------------
% 
%  SNR Analysis Toolbox Version 2.0
%
% ----------------------------------
% (snr_toolbox_batch.m)
%
...
% RAW DATA TYPE
% --------------
vd11_flag = 1; % 1 --> VD11 data, otherwise VB13-17
if vd11_flag
    addpath('./Skyra');
end
...
%  GENERAL SETTINGS
% -------------------
which_slice = 1;           % select which slice to use in rawdata with multiple slices (*ONLY for VD11 data*)
...
noise_file = [datadir 'RawData_Nova24ChCoil/meas_MID404_SNR24_Noi_BW200_FID17929.dat'];
...
if vd11_flag
    [raw,nChannels,nslices,nphases,nacq,ch_id_track] = ...
        read_measoutVD11_rl(noise_file);
    if nslices > 1
        raw = raw(:,:,:,which_slice);
    end
    noiserawdata = permute(raw,[2 1 3]); % [Frequency Phase Channel]
    noiserawdata = K_ICE_AMPL_SCALE_FACTOR*noiserawdata;
else
    [raw, noise, ref, phasecor, pc_order, centerlines, lastheader, rest, phasestab, ch_id_track] = ...
        read_meas_vb13(noise_file);
    noiserawdata = permute(raw,[1 3 2]); % [Frequency Phase Channel]
    if any(ch_id_track == 0) % assumes that if there is a "0" channel_id, it is always at the end
        if length(ch_id_track) > 1
            ch_id_track(2:end) = ch_id_track(1:end-1)+1;
            noiserawdata = noiserawdata(:,:,ch_id_track);
        end
    else
        noiserawdata = noiserawdata(:,:,ch_id_track);  % RL Dec 10,2010
    end
    noiserawdata = K_ICE_AMPL_SCALE_FACTOR*noiserawdata;
end
...
if vd11_flag
    [raw, nChannels,nslices,nphases,nacq,ch_id_track] = ...
        read_measoutVD11_rl(data_file);
    if nslices > 1
        raw = raw(:,:,:,which_slice);
    end
    signalrawdata = permute(raw,[2 1 3]); % [Frequency Phase Channel]
    signalrawdata = K_ICE_AMPL_SCALE_FACTOR*signalrawdata;
else
    [raw, noise, ref, phasecor, pc_order, centerlines, lastheader, rest, phasestab, ch_id_track] = ...
        read_meas_vb13(data_file);
    % **** NEED TO FIND OUT WHAT DATA IS IN EACH DIMENSION OF RAW (for multislice case) **** %
    % signalrawdata = squeeze(raw(:,:,:,1,1,29,1,1,2));
    % signalrawdata = permute(signalrawdata,[1 3 2]); % [Frequency Phase Channel]
    %*************************************************************************************** %
    signalrawdata = permute(raw,[1 3 2]); % [Frequency Phase Channel]
    
    if any(ch_id_track == 0) % assumes that if there is a "0" channel_id, it is always at the end
        if length(ch_id_track) > 1
            ch_id_track(2:end) = ch_id_track(1:end-1)+1;
            signalrawdata = signalrawdata(:,:,ch_id_track);
        end
    else
        signalrawdata = signalrawdata(:,:,ch_id_track);  % RL Dec 10,2010
    end
    signalrawdata = K_ICE_AMPL_SCALE_FACTOR*signalrawdata;
end
clear raw
...
%                         snr_rss(irow,icol) = signalmag / (std(noiserawdata(:))*sqrt(noisepower_test));
                        ...
                        snr_rss(irow,icol) = sqrt(2)*abs(S' * S)/sqrt((S')*noisecov*S);
