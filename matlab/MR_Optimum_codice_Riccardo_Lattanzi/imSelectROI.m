...
GP.Clims            = GetParameterValue('ColorBarLims',varargin, []);
...
if ishandle(GP.hFig)
    ...
    switch nargout
        ...
        case 1, % 1 output parameter
            ...
            temp_image = img(ROI.Xrange, ROI.Yrange, :);
            ROI.MaxVal = max(temp_image(:));
            ROI.MinVal = min(temp_image(:));
            ROI.MeanVal = mean(temp_image(:));
            clear temp_image
            varargout{1} = ROI;
