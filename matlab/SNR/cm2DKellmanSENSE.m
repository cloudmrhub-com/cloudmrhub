classdef cm2DKellmanSENSE<cm2DReconSENSE
    ...
    methods
        ...
        for icol=1:floor(np/R2)
            ...
            u(isnan(u)) = 0;
            %ac_sense_g(irow:floor(nf/R1):nf,icol:floor(np/R2):np) = reshape(sqrt(abs(diag(pinv(s'*invRn*s)).*diag(s'*invRn*s))),[current_R1 current_R2]);
            snr(irow:floor(nf/R1):nf,icol:floor(np/R2):np)=sqrt(2)*reshape(u*squeeze(imfold(irow,icol,:))./diag(sqrt(u*Rn*(u'))),[current_R1 current_R2]);
        end
        end
    
snr=this.demimicAccelerated2DImage(snr);            
        end
    end
